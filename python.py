...
# Caso contrário, cria um novo arquivo CSV vazio.
    ...
    with open("attendance/"+NAME_CSV, 'r+') as f:
        ...
        for line in myDataList:
            ...
            if len(entry) > 1:  # Certifique-se de que temos pelo menos nome e timestamp
                nameList[entry[0]] = datetime.strptime(entry[1].strip(), '%Y-%m-%d %H:%M:%S')  # Convertendo a string para um objeto datetime
...
def is_fake_via_texture(face_image):
...
while True:
    ...
    for encodeFace, faceLoc in zip(encodesCurFrame, facesCurFrame):
        ...
        matchInRecognition = check_or_update_unrecognized(encodeFace,False)
        isUnkwnown = False
        ...
        if matches[matchIndex]:
            ...
            name2 = classNames[matchIndex+1].upper()
        ...
        print(is_fake_via_texture(face_img),has_reflection(face_img))
        ...
        if isUnkwnown:
            ...
            if matchInRecognition['count'] <= MAX_CAPTURES and (last_captured_time is None or (current_time - last_captured_time).total_seconds() > CAPTURE_INTERVAL):
                ...
                matchInRecognition = check_or_update_unrecognized(encodeFace, True)
